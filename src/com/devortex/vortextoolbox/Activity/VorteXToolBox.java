...
public class VorteXToolBox extends Activity {
	...
	private static enum rmMessage {
		NOROMMANAGER, NOCWRSET, ROMMANAGEROK
	};
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Button mSwapApps = (Button) findViewById(R.id.btnSwapApps);
		...
		mSwapApps.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				launchPatchSelectorActivity();
			}
		});
	}
	
	protected void launchPatchSelectorActivity() {
		Intent i = new Intent(this, PatchSelector.class);
		startActivity(i);
	}
	...
	protected void calibrateBattery() {
		String batteryPercent = commandRunner
				.retrieveSingleCommandLineReturnLine("cat /sys/class/power_supply/battery/charge_counter");
		AlertDialog.Builder builder = new AlertDialog.Builder(_context);
		builder.setMessage(
				String.format(getString(R.string.calibrate_battery_notice),
						batteryPercent))
				.setCancelable(false)
	...
	protected void fixCWR() {
		...
		Toast toast = Toast.makeText(_context, "Fixed Always booting to CWR",
				Toast.LENGTH_SHORT);
		toast.show();
	}
	...
	private Handler preCheckHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case NOROMMANAGER:
				Editor e = ((Activity) _context).getSharedPreferences(
						PREF_FILE_NAME, Context.MODE_PRIVATE).edit();
				e.putBoolean(PREF_CWR_INSTALLED, false);
				e.commit();
				toast = Toast.makeText(_context,
						"Rom Manager not found, switching to manual mode",
						Toast.LENGTH_LONG);
				toast.show();
				break;
			case NOCWRSET:
									...
									new DialogInterface.OnClickListener() {
										public void onClick(
											...
											i.setComponent(new ComponentName(
													...
													"com.koushikdutta.rommanager.RomManager"));
											startActivity(i);
										}
									})
							.setNegativeButton("Cancel",
									new DialogInterface.OnClickListener() {
										public void onClick(
												DialogInterface dialog, int id) {
											Editor e = ((Activity) _context)
													.getSharedPreferences(
															PREF_FILE_NAME,
															Context.MODE_PRIVATE)
													.edit();
											e.putBoolean(PREF_CWR_INSTALLED,
													false);
											e.commit();
											Toast toast = Toast
													.makeText(
			...
			default:
				if (ss.getShowRMStatus()) {
					toast = Toast.makeText(_context,
							"Rom Manager Checks Out OK!", Toast.LENGTH_LONG);
					toast.show();
	...
	private void checkDeviceModel() {
		if (this.getSharedPreferences(PREF_FILE_NAME, Context.MODE_PRIVATE)
				.getBoolean(PREF_SHOW_MODEL_WARNING, true)) {
			...
			Editor e = this.getSharedPreferences(PREF_FILE_NAME,
